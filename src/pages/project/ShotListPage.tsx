...
export default function ShotListPage() {
  ...
  const [form, setForm] = useState({
    shot_number: "", description: "", shot_type: "Wide", angle: "", lens: "", movement: "Static", location_notes: "",
  });

  const resetForm = () => setForm({ shot_number: "", description: "", shot_type: "Wide", angle: "", lens: "", movement: "Static", location_notes: "" });

  const handleCreate = () => {
    if (!form.shot_number.trim()) return;
    createShot.mutate({ ...form, project_id: project.id, sort_order: shots.length }, {
  ...
  const handleEdit = (shot: Shot) => {
    setEditingShot(shot);
    setForm({ shot_number: shot.shot_number, description: shot.description || "", shot_type: shot.shot_type, angle: shot.angle || "", lens: shot.lens || "", movement: shot.movement || "Static", location_notes: shot.location_notes || "" });
    setDialogOpen(true);
  };
          ...
          <div className="space-y-2">
            {shots.map((shot, i) => (
              ...
              >
                ...
                <div className="w-16 text-sm font-mono font-medium text-primary">{shot.shot_number}</div>
      ...
      <Dialog open={dialogOpen} onOpenChange={v => { setDialogOpen(v); if (!v) setEditingShot(null); }}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4">
            <Input placeholder="Shot # (e.g. 1A)" value={form.shot_number} onChange={e => setForm(f => ({ ...f, shot_number: e.target.value }))} />
            <Textarea placeholder="Description" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
            <div className="grid grid-cols-2 gap-3">
              <Select value={form.shot_type} onValueChange={v => setForm(f => ({ ...f, shot_type: v }))}>
            ...
            <Input placeholder="Location notes" value={form.location_notes} onChange={e => setForm(f => ({ ...f, location_notes: e.target.value }))} />
            <Button onClick={editingShot ? handleUpdate : handleCreate} disabled={!form.shot_number.trim()} className="w-full">
