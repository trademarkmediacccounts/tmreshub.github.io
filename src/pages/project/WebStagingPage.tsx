...
export default function WebStagingPage() {
  ...
  const handleSave = () => {
    ...
    const payload = { ...form, url: form.url || null, notes: form.notes || null, project_id: project.id };
    if (editingId) {
  ...
  const deployNow = (e: StagingEnvironment) => {
    updateEnv.mutate({ id: e.id, status: "Live", last_deploy: new Date().toISOString() });
  };
              ...
              >
                ...
                <div className="flex items-center justify-between pt-2 border-t border-border">
                  <span className="text-xs text-muted-foreground">
                    {env.last_deploy ? `Deployed ${new Date(env.last_deploy).toLocaleDateString()}` : "Never deployed"}
                  </span>
