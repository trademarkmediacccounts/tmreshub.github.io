...
export function GearItemDialog({ open, onOpenChange, onSubmit, item }: Props) {
  const [form, setForm] = useState({ name: "", category: "Camera Kit", status: "Available", location: "", condition: "Good", reservedFor: "" });

  useEffect(() => {
    if (item) {
      setForm({ name: item.name, category: item.category, status: item.status, location: item.location || "", condition: item.condition, reservedFor: item.reservedFor || "" });
    } else {
      setForm({ name: "", category: "Camera Kit", status: "Available", location: "", condition: "Good", reservedFor: "" });
    }
  }, [item, open]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-card border-border">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div><Label>Reserved For</Label><Input placeholder="Leave blank if not reserved" value={form.reservedFor} onChange={e => setForm(f => ({ ...f, reservedFor: e.target.value }))} /></div>
