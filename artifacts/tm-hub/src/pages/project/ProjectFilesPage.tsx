...
export default function ProjectFilesPage() {
  ...
  const [form, setForm] = useState({ name: "", fileType: "Video", category: "General", fileUrl: "", notes: "" });

  const handleCreate = () => {
    if (!form.name.trim()) return;
    createAsset.mutate({ ...form, projectId: project.id }, {
      onSuccess: () => { setDialogOpen(false); setForm({ name: "", fileType: "Video", category: "General", fileUrl: "", notes: "" }); },
    });
  };
      ...
      <div className="p-8">
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {assets.map((asset, i) => {
              const Icon = typeIcons[asset.fileType] || File;
              return (
                <motion.div
                  key={asset.id}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-xs text-muted-foreground">{asset.fileType}</span>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <Input placeholder="File URL (optional)" value={form.fileUrl} onChange={e => setForm(f => ({ ...f, fileUrl: e.target.value }))} />
