...
export default function ScriptBreakdownPage() {
  ...
  const [form, setForm] = useState({ element_type: "Props", name: "", description: "", scene_reference: "" });

  const handleCreate = () => {
    if (!form.name.trim()) return;
    createBreakdown.mutate({ ...form, project_id: project.id }, {
      onSuccess: () => { setDialogOpen(false); setForm({ element_type: "Props", name: "", description: "", scene_reference: "" }); },
    });
  };

  const grouped = ELEMENT_TYPES.reduce((acc, type) => {
    const items = breakdowns.filter(b => b.element_type === type);
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        {item.scene_reference && <p className="text-xs text-muted-foreground mt-0.5">Scene: {item.scene_reference}</p>}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <Select value={form.element_type} onValueChange={v => setForm(f => ({ ...f, element_type: v }))}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>{ELEMENT_TYPES.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
            </Select>
            <Input placeholder="Name" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
            <Input placeholder="Scene reference (e.g. Scene 3)" value={form.scene_reference} onChange={e => setForm(f => ({ ...f, scene_reference: e.target.value }))} />
